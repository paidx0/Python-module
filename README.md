   * [1. 生成6位数字随机验证码](#1-生成6位数字随机验证码)
   * [2.md5加密](#2md5加密)
   * [3. 生成唯一token](#3-生成唯一token)
   * [4、发送手机验证码](#4发送手机验证码)
      * [实现逻辑](#实现逻辑)
   * [5、生成二维码](#5生成二维码)
   * [6.微信群发](#6微信群发)
   * [7、微信自动回复](#7微信自动回复)
   * [8、提取Django中model中的字段名变成字典、列表](#8提取django中model中的字段名变成字典列表)
   * [9、数据库中给表创建数据](#9数据库中给表创建数据)
   * [10.捕捉异常](#10捕捉异常)
   * [11、获取当前时间](#11获取当前时间)
   * [12、订单编号](#12订单编号)
   * [13、mysql自动填写当前时间](#13mysql自动填写当前时间)
   * [14、drf动态过滤查询](#14drf动态过滤查询)
   * [15、linux后台运行python程序](#15linux后台运行python程序)
   * [16、追加外键](#16追加外键)
   * [17、写/读CSV文件，查看是否存在，若存在就从csv中删除](#17写读csv文件查看是否存在若存在就从csv中删除)
   * [18、django中从request获取访问路径](#18django中从request获取访问路径)
   * [19、Django收集静态文件](#19django收集静态文件)
   * [20、xadmin插件](#20xadmin插件)
   * [21、uwsgi自动重启](#21uwsgi自动重启)
   * [22、谷歌浏览器快捷键](#22谷歌浏览器快捷键)
   * [23、git克隆分支](#23git克隆分支)
   * [24、mysql更新语句、新增列、删除列](#24mysql更新语句新增列删除列)
   * [25、删除指定格式的文件](#25删除指定格式的文件)
   * [26、计算文件总数](#26计算文件总数)
   * [27、计算文件夹大小](#27计算文件夹大小)
   * [28、Django实现jsonp跨域](#28django实现jsonp跨域)
   * [29、微信获取用户信息](#29微信获取用户信息)
   * [30、uwsgi初始配置问题](#30uwsgi初始配置问题)
   * [31、django中drf序列化](#31django中drf序列化)
   * [32、第三方库](#32第三方库)
   * [33、requests请求https携带CA证书](#33requests请求https携带ca证书)
   * [34、django创建缓存命令](#34django创建缓存命令)
   * [35、Django 更改超级用户密码](#35django-更改超级用户密码)
   * [36、restframe使用缓存](#36restframe使用缓存)
   * [37、数据库](#37数据库)
   * [38、linux常用命令](#38linux常用命令)
   * [39、xadmin禁止增加、删除](#39xadmin禁止增加删除)
   * [40、时间格式字符串相减](#40时间格式字符串相减)
   * [41、显示循环进度条](#41显示循环进度条)
   * [42、把列表中的字典转成csv文件](#42把列表中的字典转成csv文件)
   * [43、windows添加右键新建MarkDown文件](#43windows添加右键新建markdown文件)
   * [44、redis设置值定时过期](#44redis设置值定时过期)
   * [45、linux根据端口杀进程](#45linux根据端口杀进程)
   * [46、监控linux网络流量](#46监控linux网络流量)
   * [47、win10添加右键打开cmd](#47win10添加右键打开cmd)
   * [48、xadmin后台无法显示下拉框完整内容](#48xadmin后台无法显示下拉框完整内容)
   * [49、xadmin单点登录](#49xadmin单点登录)
   * [50、Django restful 多个models数据表序列化合并返回（一次请求返回多个序列化器数据）](#50django-restful-多个models数据表序列化合并返回一次请求返回多个序列化器数据)
   * [51、 Django序列化器返回外键关联数据](#51-django序列化器返回外键关联数据)
   * [52、python Django通过User Agent判断请求来源是微信扫一扫或者是支付宝扫一扫](#52python-django通过user-agent判断请求来源是微信扫一扫或者是支付宝扫一扫)
   * [53、xadmin后台集成’导入‘插件，导入excel文件](#53xadmin后台集成导入插件导入excel文件)
   * [54、Django中查找今天进账金额](#54django中查找今天进账金额)
   * [55、判断是什么系统](#55判断是什么系统)
   * [56、sql查询](#56sql查询)
   * [57、 xadmin后台删除数据出现错误](#57-xadmin后台删除数据出现错误)
   * [58、xdamin限制用户点击](#58xdamin限制用户点击)
   * [59、获取公众号关注url](#59获取公众号关注url)
   * [60、xadmin后台用户操作表权限](#60xadmin后台用户操作表权限)
   * [61、使用nginx部署项目](#61使用nginx部署项目)
   * [62、xadmin后台发送邮件找回密码](#62xadmin后台发送邮件找回密码)
   * [63、xadmin外键下拉框添加过滤](#63xadmin外键下拉框添加过滤)
   * [64、xadmin即时编辑器去掉空标签](#64xadmin即时编辑器去掉空标签)
   * [65、用户增加的小组件，让其他用户可见](#65用户增加的小组件让其他用户可见)
   * [66、pip install uwsgi出错](#66pip-install-uwsgi出错)
   * [66、xadmin后台加载数据慢，解决方案](#66xadmin后台加载数据慢解决方案)
   * [67 、xadmin导出插件处理，增加导出勾选数据项](#67-xadmin导出插件处理增加导出勾选数据项)
         * [1、加载js文件](#1加载js文件)
         * [2、修改export.py，后端处理下载文件](#2修改exportpy后端处理下载文件)
   * [68、使用F查询更新用户重要数据](#68使用f查询更新用户重要数据)
   * [69、日志输出模块](#69日志输出模块)
   * [70、字典排序](#70字典排序)
      * [对列表中字典按多个键值排序](#对列表中字典按多个键值排序)
   * [71、时间格式与字符串互转、比较大小](#71时间格式与字符串互转比较大小)
   * [72、python把’\u’开头的字符串转中文](#72python把u开头的字符串转中文)
   * [73、django进行数据迁移时出现No changes detected解决方案](#73django进行数据迁移时出现no-changes-detected解决方案)
   * [74、ubuntu下载文件到windows桌面](#74ubuntu下载文件到windows桌面)
   * [75、git查看提交日志](#75git查看提交日志)
   * [76、python翻译模块](#76python翻译模块)
   * [77、python实现字符串转字典](#77python实现字符串转字典)
   * [78、django app 如何在后台显示中文名](#78django-app-如何在后台显示中文名)
   * [79、使用python给Excel指定行添加数据](#79使用python给excel指定行添加数据)
   * [80、python中的format格式拼接字符串](#80python中的format格式拼接字符串)
   * [81、数据库inner join、full join、left join、union、union all区别](#81数据库inner-joinfull-joinleft-joinunionunion-all区别)
   * [82、windows电脑查看端口使用情况](#82windows电脑查看端口使用情况)
   * [83、Django生成表和反向生成Model](#83django生成表和反向生成model)
   * [84、windows的hosts文件位置](#84windows的hosts文件位置)
   * [85、postgresql数据库](#85postgresql数据库)
   * [86、python控制台输出带颜色的文字方法](#86python控制台输出带颜色的文字方法)
   * [87、控制台输出白色方框](#87控制台输出白色方框)
   * [88、xadmin后台编辑多对多字段](#88xadmin后台编辑多对多字段)
   * [89、Django后台通过按钮显示TextField字段内容](#89django后台通过按钮显示textfield字段内容)
   * [90、前端判断上传的文件是否为图片且限制大小为300kB](#90前端判断上传的文件是否为图片且限制大小为300kb)
   * [91、form表单提交前验证](#91form表单提交前验证)
   * [92、有序字典](#92有序字典)
   * [93、Python在Windows系统下实现TTS（文字转语音）](#93python在windows系统下实现tts文字转语音)
   * [94、定义一个简单闹钟](#94定义一个简单闹钟)
   * [95、根据年月获取当月天数](#95根据年月获取当月天数)
   * [96、Django获取favicon.ico图标](#96django获取faviconico图标)
   * [97、django 静态文件配置](#97django-静态文件配置)
   * [98、django JsonResponse返回中文时显示unicode编码（\u67e5\u8be2）](#98django-jsonresponse返回中文时显示unicode编码u67e5u8be2)
   * [99、获取当前时间月份的首日与最后一天](#99获取当前时间月份的首日与最后一天)
   * [100、xadmin后台批量操作](#100xadmin后台批量操作)
   * [101、xadmin过滤器外键显示特定值（比如只能过滤自己与超级管理员定义的数据）](#101xadmin过滤器外键显示特定值比如只能过滤自己与超级管理员定义的数据)
   * [102、Django中的CSRF局部禁用](#102django中的csrf局部禁用)
   * [103、使用python命令创建django项目](#103使用python命令创建django项目)
   * [104、 二维列表、二维数组行求和与列求和](#104-二维列表二维数组行求和与列求和)
   * [105、获取时间字符串的月份数](#105获取时间字符串的月份数)
   * [106、字符串不足补零](#106字符串不足补零)
   * [107、时间戳转字符串日期](#107时间戳转字符串日期)
   * [108、列表转字符串](#108列表转字符串)
   * [109、pip国内镜像源](#109pip国内镜像源)
   * [110、python把时间字符串转换成刚刚、1天前、3个月前、1年前](#110python把时间字符串转换成刚刚1天前3个月前1年前)
   * [111、Django使用ORM执行sql语句](#111django使用orm执行sql语句)
   * [112、Django通过变量字段获取数据](#112django通过变量字段获取数据)
   * [113、Django查询变量字段、更新变量字段](#113django查询变量字段更新变量字段)
   * [114、vue生成dist文件](#114vue生成dist文件)
   * [115、python获取电脑磁盘、CPU、内存使用情况](#115python获取电脑磁盘cpu内存使用情况)
   * [116、max比较字典列表](#116max比较字典列表)
   * [117、django查询表的具体字段](#117django查询表的具体字段)
   * [118、MySQL查询数据添加自增序号](#118mysql查询数据添加自增序号)
   * [119、MySQL 时间截取年月日](#119mysql-时间截取年月日)
   * [120、 列表取奇数下标值](#120-列表取奇数下标值)
   * [121、列表取偶数下标值](#121列表取偶数下标值)
   * [122、列表相同元素分类、分组](#122列表相同元素分类分组)
   * [123、ubuntu切换用户](#123ubuntu切换用户)
   * [124、numpy二维数组获取某一列](#124numpy二维数组获取某一列)
